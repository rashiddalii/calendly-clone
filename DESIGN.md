# DESIGN.md — Indigo Scheduling Hub
> Exported from Google Stitch · Project: `3922679111200956344` · Last updated: 2026-04-15
...
| Token | Hex | Usage |
|-------|-----|-------|
| `primary` | `#4a4bd7` | Interactive elements, links |
| `primary-container` | `#7073ff` | CTA gradient endpoint, hover states |
| `primary-fixed` | `#7073ff` | Fixed primary tint |
| `primary-fixed-dim` | `#6366f1` | Brand color anchor |
| `on-primary` | `#fbf7ff` | Text on primary backgrounds |
| `on-primary-container` | `#000000` | Text on primary containers |

**CTA Gradient Rule:** Never use a flat primary hex on buttons. Apply `linear-gradient(135deg, #4a4bd7, #7073ff)` for a sense of forward motion.

### Surface Hierarchy (Stack these like layers of fine paper)
| Level | Token | Hex | Use For |
|-------|-------|-----|---------|
| 0 — Base | `surface` | `#fcf8fe` | Page background |
| 1 — Sections | `surface-container-low` | `#f6f2fb` | Sidebars, panels |
| 1 — Sections | `surface-container` | `#f0ecf6` | Section backgrounds |
| 2 — Cards | `surface-container-lowest` | `#ffffff` | Cards, inputs (active) |
| Elevated | `surface-container-high` | `#eae7f1` | Input resting state |
| Floating | `surface-container-highest` | `#e4e1ed` | Active overlays, tertiary buttons |
| Dimmed | `surface-dim` | `#dbd8e4` | Disabled / muted areas |
| Bright | `surface-bright` | `#fcf8fe` | Highlighted panels |

### Text & On-Surface
| Token | Hex | Usage |
|-------|-----|-------|
| `on-surface` | `#32323b` | Primary text (NEVER use pure #000000) |
| `on-surface-variant` | `#5f5e68` | Secondary metadata, labels |
| `on-background` | `#32323b` | Body text on page background |
| `background` | `#fcf8fe` | Document background |

### Secondary & Tertiary
| Token | Hex | Usage |
|-------|-----|-------|
| `secondary` | `#5d5d72` | Secondary actions |
| `secondary-container` | `#e2e0f9` | Tentative holds, badges |
| `on-secondary` | `#fbf7ff` | Text on secondary |
| `on-secondary-container` | `#505064` | Text on secondary container |
| `tertiary` | `#745479` | Accents, highlights |
| `tertiary-container` | `#f5cdf9` | Tertiary backgrounds |
...
| Token | Hex | Usage |
|-------|-----|-------|
| `outline` | `#7b7984` | Visible boundaries (use sparingly) |
| `outline-variant` | `#b3b0bc` | Ghost borders at 15% opacity only |
| `surface-tint` | `#4a4bd7` | Brand tint for empty states |
...
Place a `surface-container-lowest` (#ffffff) card on a `surface-container-low` (#f6f2fb) section. The contrast creates a natural "lift" without shadows.

### Shadow Tokens (for floating elements only)
| Use Case | Value |
|----------|-------|
| Modals, Popovers | `box-shadow: 0 12px 40px rgba(50, 50, 59, 0.06)` |
| Hover cards | `box-shadow: 0 4px 16px rgba(50, 50, 59, 0.04)` |

**Ghost Border Fallback:** If a container requires a boundary for accessibility, use `outline-variant` (#b3b0bc) at **15% opacity** only. Never 100% opaque borders.
...
|---------|-----------|------|--------|-------|
| **Primary** | `linear-gradient(135deg, #4a4bd7, #7073ff)` | `#fbf7ff` | none | Darken 5% |
...
- Natural lift via parent background: `surface-container-low` (#f6f2fb)

### Input Fields
| State | Background | Border |
|-------|-----------|--------|
| Resting | `surface-container-high` (#eae7f1) | none |
...
- Radius: `md` (0.75rem)
- Label: `label-md` in `on-surface-variant` (#5f5e68)
...
- **Urgent/Conflict:** `tertiary` (#745479) or `error` (#a8364b)
- **Tentative:** `secondary` (#5d5d72)

### Navigation / Sidebar
- Background: `surface-container-low` (#f6f2fb)
...
- Shadow: `0 12px 40px rgba(50, 50, 59, 0.06)`
- Radius: `lg` (1rem)
...
### Do
- Use `on-surface` (`#32323b`) for all body text — never pure `#000000`
...
- Use `surface-tint` (#4a4bd7) for subtle brand presence in empty states
...
- `on-surface` (#32323b) on `surface` (#fcf8fe) passes AA at all sizes
