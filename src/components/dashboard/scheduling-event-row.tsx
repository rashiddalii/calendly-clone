...
function locationLabel(location: string): string {
  return LOCATION_LABELS[location as LocationValue] ?? location;
}
...
export function SchedulingEventRow({
  ...
  const metaLine = `${eventType.duration} min · ${locationLabel(eventType.location ?? "google_meet")} · One-on-one`;
  ...
  return (
    <>
      <ConfirmationDialog
        ...
        description="This removes the event type from your scheduling page. Existing bookings stay in your meetings list."
        confirmLabel="Delete event type"
        cancelLabel="Keep event type"
        isPending={isPending}
        onConfirm={handleDelete}
      />
      <article className="group flex flex-col gap-3 overflow-hidden rounded-xl border border-[#E5E7EB] bg-white p-4 shadow-sm transition-shadow hover:shadow-md sm:flex-row sm:items-stretch sm:gap-0 sm:p-0 sm:pr-4">
        <div
        ...
        <div className="flex shrink-0 flex-wrap items-center justify-end gap-2 sm:gap-1.5 sm:py-4">
          <Button
            ...
            variant="outline"
            size="sm"
            className="hidden h-9 gap-1.5 rounded-full border-[#E5E7EB] bg-white px-3.5 text-xs font-semibold text-[#374151] hover:bg-[#F9FAFB] sm:inline-flex"
            onClick={handleCopy}
          >
            {copied ? (
              <>
          ...
          <Button
            ...
            variant="ghost"
            size="icon-sm"
            className="hidden text-[#6B7280] hover:bg-[#F3F4F6] sm:inline-flex"
            aria-label="Share"
            onClick={handleShare}
          >
