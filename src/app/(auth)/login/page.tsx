...
const HIGHLIGHTS = [
  {
    icon: CalendarClock,
    title: "Today’s schedule, instantly",
    body: "Open Fluid and see what’s on your calendar without digging through invites.",
  },
  {
    icon: Shield,
    title: "Sign-in you can trust",
    body: "Magic links expire quickly; Google and Microsoft use the same OAuth you already rely on.",
  },
  {
    icon: Sparkles,
  ...
  return (
    <div className="flex min-h-screen flex-col bg-white text-[#0F172A]">
      ...
      <div className="grid flex-1 lg:grid-cols-2">
        {/* Left: narrative (desktop left; mobile below form) */}
        <div
          className="order-2 flex flex-col justify-center border-t border-[#E5E7EB] px-4 py-14 sm:px-10 lg:order-1 lg:border-t-0 lg:border-r lg:px-14 xl:px-20"
            ...
            <ul className="mt-10 flex flex-col gap-6">
              {HIGHLIGHTS.map(({ icon: Icon, title, body }) => (
                <li key={title} className="flex gap-4">
                  <span
                    className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-white shadow-[0_1px_0_#E5E7EB]"
                    style={{ color: BLUE }}
                  >
                    <Icon className="h-5 w-5" strokeWidth={2} aria-hidden />
                  </span>
                  <div>
                    <p className="font-[family-name:var(--font-manrope)] text-[17px] font-semibold text-[#0F172A]">
                      {title}
                    </p>
                    <p className="mt-1 text-[15px] leading-snug text-[#64748B]">
                      {body}
                    </p>
                  </div>
                </li>
        ...
        {/* Right: form */}
        <div className="order-1 flex flex-col justify-center px-4 py-12 sm:px-10 lg:order-2 lg:px-16 xl:px-24">
          <div className="mx-auto w-full max-w-[440px]">
            <h2 className="font-[family-name:var(--font-manrope)] text-3xl font-bold tracking-tight text-[#0F172A] sm:text-[2rem] sm:leading-tight">
              Log in to Fluid
            </h2>
